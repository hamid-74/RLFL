...
clients_1234 = create_clients(X_train_1234, y_train_1234, num_clients=63, initial='client_1234')

clients_rest = create_clients(X_train_rest, y_train_rest, num_clients=96, initial='client_rest')
...
if(os.path.exists('keras_fmnist')):
    ...
    value_network = tf.keras.models.load_model('keras_fmnist')
else:
...
if(os.path.exists('epoch_fmnist.txt')):
    with open('epoch_fmnist.txt', 'r') as file:
...
# runs = [   'run10', 'run11','run12', 'run13','run14', 'run15','run16', 'run17', 'run18', 'run19',]
runs = [    'run110','run111','run112', 'run113','run114', 'run115','run116' ]
...
for run in runs:
        ...
        if(comm_round > 2):
            ...
            if len(replay_RL)>batch_RL:
                with tf.GradientTape() as tape:
                    ...
                    episode_RL += 1
                    value_network.save('keras_fmnist/')
                    if epoch_RL%100==0:
                        epsilon_RL*=0.9
                    epoch_RL+=1

                    with open('epoch_fmnist.txt', 'w') as file:
    ...
    with open('results/fmnist/RL/accs_all_clients_NIID_RL_' + run + '.json', 'w') as file:
        json.dump(accs_all_clients, file)

    with open('results/fmnist/RL/global_accs_NIID_RL_' + run + '.json', 'w') as file:
        json.dump(global_accs, file)

    with open('results/fmnist/RL/actions_RL_' + run + '.json', 'w') as file:
    ...
    iid_df.to_csv('results/fmnist/RL/MNIST_NIID_RL_'  + run + '.csv',index=False)
