...
result_folder_name = 'results/mnist_Non_IID_1/'
...
def dump_stats(global_acc_loss_action, global_accs, accs_all_clients, max_acc_achieved,folder_name, run, start_point):

    with open(folder_name + 'global_acc_loss_action_' + start_point + '_' + run +'.json', 'w') as file:
        json.dump(global_acc_loss_action, file)

    
    with open(folder_name + 'global_accs_' + start_point + '_' + run +'.json', 'w') as file:
        json.dump(global_accs, file)

    with open(folder_name + 'accs_clients_' + start_point + '_' + run +'.json', 'w') as file:
        json.dump(accs_all_clients, file)
    
    with open(folder_name + 'max_acc_' + start_point + '_' + run +'.json', 'w') as file:
...
for run in runs:
    ...
    for comm_round in range(comms_round):
        ...
        for(X_test, Y_test) in test_batched:
            ...
            print('ternary_scale: {} | run: {} | start_point: {}'.format(ternary_scale, run, start_point))
    ...
    dump_stats(global_acc_loss_action, global_accs, accs_all_clients, max_acc_achieved, result_folder_name, str(start_point), run)
