...
if(os.path.exists('replay_RL.pkl')):
    with open('replay_RL.pkl', 'rb') as file:
...
runs = [   'run100', 'run101','run102', 'run103','run104', 'run105','run106', 'run107', 'run108', 'run109',]
...
for run in runs:
    ...
    #         print('client_names', client_names, len(client_names))
        ...
        if(comm_round > 2):
            ...
            with open('replay_RL.pkl', 'wb') as file:
            ...
            if len(replay_RL)>batch_RL:
                with tf.GradientTape() as tape:
                    ...
                    with open('epsilon.txt', 'w') as file:
    ...
    with open('results/RL/global_accs_NIID_RL_' + run + '.json', 'w') as file:
        json.dump(global_accs, file)

    with open('results/RL/actions_RL_' + run + '.json', 'w') as file:
    ...
    iid_df.to_csv('results/RL/MNIST_NIID_RL_'  + run + '.csv',index=False)
