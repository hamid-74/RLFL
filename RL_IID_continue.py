...
if(os.path.exists('keras_IID')):
    print('loading saved model')
    value_network = tf.keras.models.load_model('keras_IID')
else:
...
if(os.path.exists('replay_RL_IID.pkl')):
    with open('replay_RL_IID.pkl', 'rb') as file:
        replay_RL = pickle.load(file)
else:
    replay_RL = deque(maxlen=5000)

if(os.path.exists('epoch_IID.txt')):
    with open('epoch_IID.txt', 'r') as file:
        epoch_RL = int(file.read())
else: 
    epoch_RL = 0

if(os.path.exists('epsilon_IID.txt')):
    with open('epsilon_IID.txt', 'r') as file:
...
# runs = [  'run86', 'run87', 'run88', 'run89']
# runs = [ 'run13','run14', 'run15','run16', 'run17', 'run18', 'run19']
# runs = [   'run0', 'run1','run2', 'run3','run4', 'run5','run6', 'run7', 'run8', 'run9']
runs = [     'run55','run56', 'run57', 'run58', 'run59','run60', 'run61','run62']
# runs = [  'run59', 'run60','run61','run62', 'run63', 'run64', 'run65' ]
...
for run in runs:
    ...
    #         print('client_names', client_names, len(client_names))
        ...
        if(comm_round > 2):
            ...
            with open('replay_RL_IID.pkl', 'wb') as file:
            ...
            if len(replay_RL)>batch_RL:
                with tf.GradientTape() as tape:
                    ...
                    episode_RL += 1
                    value_network.save('keras_IID/')
                    if epoch_RL%100==0:
                        epsilon_RL*=0.995
                    epoch_RL+=1

                    with open('epoch_IID.txt', 'w') as file:
                        file.write(str(epoch_RL))

                    with open('epsilon_IID.txt', 'w') as file:
    ...
    with open('results/RL_IID/accs_all_clients_IID_RL_' + run + '.json', 'w') as file:
        json.dump(accs_all_clients, file)

    with open('results/RL_IID/global_accs_IID_RL_' + run + '.json', 'w') as file:
        json.dump(global_accs, file)

    with open('results/RL_IID/actions_RL_' + run + '.json', 'w') as file:
    ...
    iid_df.to_csv('results/RL_IID/MNIST_IID_RL_'  + run + '.csv',index=False)
